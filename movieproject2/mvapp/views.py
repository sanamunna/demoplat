...
def movie_list_by_category(request, category_slug=None):
...
def modify(request,movie_slug):
    ...
    if movie.user!=request.user:
        return HttpResponseForbidden("you can modify your own movies!")
...
def delete(request,movie_slug):
    if request.method == 'POST':
        movie=Movie.objects.get(slug=movie_slug)
        if movie.user != request.user:
            return HttpResponseForbidden("you can delete your own movies!")
        movie.delete()
        return redirect('/')
